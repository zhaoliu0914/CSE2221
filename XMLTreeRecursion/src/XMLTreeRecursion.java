...
public final class XMLTreeRecursion {
    ...
    private static int tagCount(XMLTree xml, String tag) {
        ...
        if (xml.isTag()) {
            String labelName = xml.label();
            if (labelName.equals(tag)) {
                count++;
            }
            int numberOfChildren = xml.numberOfChildren();
            for (int i = 0; i < numberOfChildren; i++) {
                XMLTree node = xml.child(i);
                if (node.isTag()) {
                    count = count + tagCount(node, tag);
                }
            }
        }
        return count;
    ...
    private static void outputTextNodes(XMLTree xml, SimpleWriter out) {
        ...
        if (xml.isTag()) {
            ...
            for (int i = 0; i < numberOfChildren; i++) {
                XMLTree node = xml.child(i);
                outputTextNodes(node, out);
            }
        } else {
            String textContent = xml.label();
    ...
    private static void outputXML(XMLTree xml, SimpleWriter out,
        ...
        if (xml.isTag()) {
            ...
            int numberOfChildren = xml.numberOfChildren();
            int nextTab = indentationLevel + 1;
            for (int i = 0; i < numberOfChildren; i++) {

                XMLTree child = xml.child(i);

                outputXML(child, out, nextTab);
            }
