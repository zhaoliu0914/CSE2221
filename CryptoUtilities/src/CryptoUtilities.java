...
public final class CryptoUtilities {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (remainder.isZero()) {
            ...
            NaturalNumber temp = new NaturalNumber2(n.toString());
            n.multiply(temp);

            NaturalNumber moduloRemainder = n.divide(m);
            n.copyFrom(moduloRemainder);
        } else {
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber one = new NaturalNumber2("1");
        NaturalNumber two = new NaturalNumber2("2");
        NaturalNumber result = new NaturalNumber2(w.toString());

        NaturalNumber firstBoundary = new NaturalNumber2(n.toString());
        firstBoundary.subtract(two);
        // check for first fact
        // (w^2 mod n) = 1, and w is located in [2, n–2]
        // w is a witness that n is composite, when (w^2 mod n) = 1
        if (result.compareTo(firstBoundary) <= 0) {
            powerMod(result, two, n);
        ...
        // check for second fact
        // (w^(n-1) mod n) ≠ 1, and w is located in [1, n–1]
        // w is a witness that n is composite, when (w^(n-1) mod n) ≠ 1
        if (!isWitness) {
            result.copyFrom(w);
            NaturalNumber tempN = new NaturalNumber2(n.toString());
            tempN.decrement();

            powerMod(result, tempN, n);
        ...
        /*
         * NaturalNumber indexW = new NaturalNumber2(w.toString());
         * NaturalNumber temp = new NaturalNumber2(); NaturalNumber
         * firstBoundary = new NaturalNumber2(n.toString());
         * firstBoundary.subtract(two); NaturalNumber secondBoundary = new
         * NaturalNumber2(n.toString()); secondBoundary.decrement(); while
         * (indexW.compareTo(secondBoundary) <= 0 && !isWitness) { // check for
         * first fact // (w^2 mod n) = 1, and w is located in [2, n–2] // w is a
         * witness that n is composite, when (w^2 mod n) = 1 if
         * (indexW.compareTo(firstBoundary) <= 0) { temp.copyFrom(indexW);
         * powerMod(temp, two, n); if (temp.compareTo(one) == 0) { isWitness =
         * true; } } // check for second fact // (w^(n-1) mod n) ≠ 1, and w is
         * located in [1, n–1] // w is a witness that n is composite, when
         * (w^(n-1) mod n) ≠ 1 temp.copyFrom(indexW); powerMod(temp,
         * secondBoundary, n); if (temp.compareTo(one) != 0) { isWitness = true;
         * } indexW.increment(); }
         */
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        final int maxCandidate = 50;
        NaturalNumber one = new NaturalNumber2("1");
        NaturalNumber nMinusOne = new NaturalNumber2(n.toString());
        nMinusOne.decrement();
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            // check for the interval [2, n–1]
            // if cannot find number in the interval [2, n–1] a witness that is composite
            // then this number with high probability but not certainty is prime.

            for (int i = 0; i < maxCandidate && isPrime; i++) {
                // generate random number
                // this random number should 1 < number < n-1
                NaturalNumber number = new NaturalNumber2();
                do {
                    NaturalNumber temp = randomNumber(n);
                    number.transferFrom(temp);
                } while (number.compareTo(one) <= 0
                        || number.compareTo(nMinusOne) >= 0);

                boolean isWitness = isWitnessToCompositeness(number, n);
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        NaturalNumber two = new NaturalNumber2("2");
        NaturalNumber index = new NaturalNumber2(n.toString());
        if (isEven(index)) {
            index.increment();
        }
        // looking for the next prime number.
        while (!isPrime) {
            isPrime = isPrime2(index);
            if (isPrime) {
                n.copyFrom(index);
            }
